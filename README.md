...
- **Team standards** — natural-language coding standards (built-in packs like `effective-java`/`effective-go`/`react-hooks`/`owasp-top10`, an org baseline repo, and per-repo `.lanekeeper/guidelines/*.md`), path-scoped and injected into the agents only when matching files change. Comments cite the guideline that drove them.
